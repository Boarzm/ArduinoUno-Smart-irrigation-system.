...
/**
 * @brief Controls irrigation for a single garden based ONLY on soil moisture (two-state: DRY or WET).
 * @param moisturePin Unused but maintained for original function signature.
 * @param valvePin Digital pin controlling the solenoid valve.
 * @param ledPin Digital pin controlling the status LED.
 * @param gardenId A simple identifier (1 or 2) for logging/display.
 * @param moistureReading Analog reading from the moisture sensor (High = Dry).
 */
void control_garden(int moisturePin, int valvePin, int ledPin, int gardenId, int moistureReading) {
  
  // Logic: Check if the soil is dry (reading is high)
  if (moistureReading >= DRY_THRESHOLD) {
    // STATE: DRY - Start Irrigation
...
void setup() {
  ...
  Serial.println("Logic: IRRIGATE only if [Soil Moisture is DRY]"); // Updated Logic message
...
void loop() {
  ...
  // Garden 1 Control
  // The '1' here is the gardenId passed to the function for logging.
  control_garden(MOISTURE_1_PIN, VALVE_1_PIN, LED_1_PIN, 1, moisture_1_value);

  // Garden 2 Control
  // The '2' here is the gardenId passed to the function for logging.
  control_garden(MOISTURE_2_PIN, VALVE_2_PIN, LED_2_PIN, 2, moisture_2_value);
