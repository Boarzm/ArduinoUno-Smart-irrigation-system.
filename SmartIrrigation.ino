...
void control_garden(int moisturePin, int valvePin, int ledPin, int gardenId, int moistureReading) {
  // Check if the soil is dry (reading is high)
...
void update_lcd() {
  ...
  // Check the actual state of the valve, which reflects the two-state logic
...
void loop() {
  ...
  if (isnan(humidity) || isnan(temperature)) {
    ...
    temperature = 0.0; // Reset or keep previous value
    humidity = 0.0;
  }
